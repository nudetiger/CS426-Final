...
\begin{longtable}{L{4.3cm} L{1.8cm} X}
\toprule
...
The gateway. Roughly 230 lines, no database, no state: it holds the API key, builds the system prompt for
the requested mode, normalises upstream errors, and relays the token stream. \\
\addlinespace
...
\begin{itemize}
  \item \textbf{No dependency-injection framework.} Hilt or Koin would earn their keep in a larger team
        project; here the object graph is nine lazily-created singletons on the \code{Application}, which
...
phase order, and the invariant that the AI subsystem may never reach a DAO. \code{docs/plan.md} is the
