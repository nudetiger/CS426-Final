...
\usepackage{tabularx}
% xltabular = longtable page breaking + tabularx X columns. Plain longtable does
% not understand X, so every multi-page table with a flexible column uses this.
...
% Long \code{} spans and identifiers are unbreakable, so give TeX room to stretch a
% line rather than let it run into the margin.
